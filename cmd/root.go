...
var rootCmd = cobra.NewRootCommand(func(c *cobra.Command) {
	...
	c.PersistentFlags().Uint16P("size", "r", 20, "download size for each thread")
	c.PersistentFlags().Uint16P("threads", "t", 3, "download threads for each type, each thread will took 2MB traffic")
